...
type MemoryAgentResult
  = | { ok: true; data: { response?: string } & Record<string, unknown> }
    | { ok: false; status: number; errorMsg: string };

export class MemoryPlugin implements ContextPlugin {
  readonly name = 'memory';

  constructor(private opts: MemoryPluginOptions) {}
    ...
    return [
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            channel: {
              type: 'string',
              description: '消息来源渠道（取自 source 属性），用于定位对话上下文',
            },
          },
          required: ['query'],
        },
        execute: async (args: Record<string, unknown>) => {
          ...
          const result = await this.callMemoryAgent(
            '/query',
            {
              userId: ctx.userId,
              query,
              context: getOptionalStringArg(args, 'context') || '',
              chatJid: getOptionalStringArg(args, 'channel') || ctx.chatJid,
          ...
          return { content: result.data.response || '没有找到相关记忆。' };
        },
      },
      {
        ...
        execute: async (args: Record<string, unknown>) => {
          ...
          const result = await this.callMemoryAgent(
            '/remember',
            {
              userId: ctx.userId,
              content,
              importance: getOptionalStringArg(args, 'importance') || 'normal',
              chatJid: getOptionalStringArg(args, 'channel') || ctx.chatJid,
              groupFolder: ctx.groupFolder,
            },
  ...
  private buildHomeMemoryPrompt(ctx: PluginContext): string {
    ...
    const personalityContent = tryReadFileContent(
      path.join(workspaceMemoryIndex, 'personality.md'),
    );
  ...
  /**
   * Read-only memory prompt for non-home group containers.
   */
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (status === 408) {
          ...
          errorMsg = '上一个记忆查询还在处理中，稍等一下';
        }
        return { ok: false, status, errorMsg };
      }

      const data = (await res.json()) as { response?: string } & Record<
        string,
        unknown
      >;
      return { ok: true, data };
...
function tryReadFileContent(filePath: string): string {
  try {
    ...
    // Ignore optional memory attachment read failures.
  }
