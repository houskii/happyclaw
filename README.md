...
本项目是 [HappyClaw](https://github.com/riba2534/happyclaw) 的实验性 fork，目标不是只给上游加一个新 Provider，而是把系统重心调整为：

1. **Codex / Claude 双 Provider 共存** — 工作区可切换执行 Provider，系统默认值、工作区覆盖、宿主机/容器注入都能分别管理
2. **Codex-first 的本机部署路径** — 支持直接复用 `codex login` 的本地登录态，先在宿主机模式把系统跑通，再决定是否开启 Docker
3. **Memory Agent 长期记忆体系** — 将记忆从 inline MCP 工具提升为 per-user 子进程，独立维护索引、知识和会话整理
4. **显式消息路由** — Agent 的 stdout 只服务 Web；飞书 / Telegram / QQ 的最终回复必须通过 `send_message(channel=...)` 显式送达
5. **更强的 Agent 自主性** — Skills 由 Agent 直接在文件系统中创建和管理，不再依赖注册表安装流

这意味着它已经不是“原版 HappyClaw + Codex 选项”这么简单，而是一个以 **多 Provider、自主路由、长期记忆、会话隔离** 为核心的新运行模型。上游能力会按需选择性合并，但这份 README 优先解释这个 fork 当前真正成立的系统边界和运行方式。

## 与原版 HappyClaw 的主要差异

这份 README 和上游 README 的区别，不只是“多了几个 fork 特性”：

- **上游 README 更偏通用产品介绍**；这份 README 更强调 fork 后的执行模型、目录约定、Provider 行为与运维落地路径
- **上游默认读者是假设 Claude 主链路**；这份 README 把 **Codex-first** 作为一等部署路径来写
- **上游侧重功能面**；这份 README 明确解释 **哪些能力是 fork 新增的、哪些行为已经和上游不兼容**
- **上游不会讲 Codex App 与本地 Codex 状态协作**；这份 README 会说明本 fork 已形成的 Codex 生态适配能力

如果你已经熟悉原版 HappyClaw，可以把这个 fork 理解成以下几项结构性变化：

| 维度 | 上游 README 的默认心智 | 这个 fork 的实际心智 |
|------|----------------------|--------------------|
| Provider | 偏 Claude 单链路 | Claude / Codex 双链路，且 Codex 是推荐起步方式之一 |
| 记忆 | 会话级 + 工具级 | per-user Memory Agent + 会话私有记忆 + 全局索引 |
| IM 回复 | Agent stdout 常被视为主输出 | Web 与 IM 明确分流，IM 必须显式 `send_message` |
| 会话目录 | 以单会话运行体验为主 | 强调 `data/groups` / `data/sessions` / `data/ipc` 的强隔离布局 |
| 适配范围 | Web + IM + Claude | Web + IM + Claude + Codex + Codex App 中转与回写 |

## Codex 适配总览

这部分是当前 fork 相比原 README 最重要的新信息。Codex 支持已经不只是“能调用一下 SDK”，而是形成了一条完整的执行链路。

### 已完成的整体适配工作

1. **Provider 级抽象**  
   Claude 和 Codex 都有自己的配置、默认模型、SDK Base URL、用量接口、工作区覆盖字段和运行时注入逻辑。

2. **Codex Runner 与线程持久化**  
   容器 / 宿主机运行时会启动 `CodexRunner`，并将 thread id、rollout 和会话目录持久化，便于重启后继续恢复。

3. **Codex SDK 事件适配到统一 StreamEvent**  
   Codex 的 `thread.started` / `item.*` / `turn.completed` 会转换成 HappyClaw 自己的流式事件模型，前端不需要为 Claude / Codex 分别做一套渲染协议。

4. **系统默认值 + 工作区覆盖**  
   Codex 相关参数已经进入系统设置和工作区设置，包含：
   - 默认模型
   - 默认推理强度
   - 默认服务档位（`Standard` 通过“不传参”表示；显式支持 `Fast` / `Flex`）
   - 工作区级 `codex_model` / `codex_thinking_effort` / `codex_service_tier`

5. **凭据模式双支持**  
   Codex Provider 既支持直接复用本机 `codex login` 的 `~/.codex/auth.json`，也支持 OpenAI API Key / Base URL 模式。

6. **Codex App 协作能力**  
   已支持从本地 Codex App 最近活跃线程导入上下文，创建 HappyClaw 工作区执行侧线任务，再将结果回写回原线程。

7. **跨 Provider 协同**  
   运行中的 Agent 可以通过 `invoke_agent` 在 Claude 和 Codex 之间发起一次性子任务调用，形成跨模型协作。

### 形成的 Codex 能力

| 能力 | 状态 | 说明 |
|------|------|------|
| Codex CLI 登录态复用 | 已支持 | 直接读取本机 `~/.codex/auth.json` |
| OpenAI API Key / Base URL | 已支持 | 适合服务器或兼容网关部署 |
| 宿主机模式运行 Codex | 已支持 | 推荐本机最小验证路径 |
| 容器模式运行 Codex | 已支持 | 通过 agent-runner + `CODEX_HOME` 持久化 |
| 工作区级模型/推理强度覆盖 | 已支持 | `codex_model` / `codex_thinking_effort` |
| 工作区级服务档位覆盖 | 已支持 | `codex_service_tier`，显式支持 `fast` / `flex` |
| 系统默认值继承 | 已支持 | 新工作区与辅助查询链路可继承默认值 |
| Codex App 中转导入 | 已支持 | 从最近活跃线程导入上下文 |
| 结果回写 Codex App | 已支持 | 将执行摘要回写到来源线程 |
| Vision / 本地图片输入 | 已支持 | Codex SDK 使用本地文件路径输入图片 |
| 统一流式前端展示 | 已支持 | 通过 StreamEvent 适配层接入 |
| Mid-query push | 不支持 | Codex turn 是独立执行模型，消息会在下一轮合并 |
| 运行时权限模式切换 | 不支持 | 与 Claude 的能力边界不同 |
| 原生成本统计 | 暂未完整支持 | 当前 SDK usage 不直接给出成本，系统仍以统一 usage 框架记录 token |

### 这意味着什么

如果你今天把这个仓库部署起来，Codex 不再只是“备用 Provider”：

- 你可以把 **Codex 作为默认工作区 Provider**
- 你可以把 **Codex CLI 登录态** 当作本机最小启动路径
- 你可以在 **系统设置 / 工作区设置** 中稳定管理 Codex 参数
- 你可以把 HappyClaw 当作 **Codex App 的侧线执行器**
- 你可以在 Claude 与 Codex 之间做 **跨 Provider 协作调用**
...
- 系统默认项：模型、推理强度、服务档位都可以在 `设置 → 系统参数` 中统一配置
...
3. **Provider 自身默认**
   - CLI 登录态下由本地 Codex 配置 / 默认值决定
   - API Key 模式下由当前激活的 OpenAI Profile 决定

### 关于 Standard / Fast / Flex

Codex CLI 底层的 `service_tier` 当前不是 `standard | fast | flex` 三选一，而是：

- `fast`
- `flex`
- **不传 `service_tier`**：表示标准档，也就是你在界面里看到的 `Standard`

因此在这个 fork 里，系统和工作区配置会这样映射：

- `Standard`：不显式传 `service_tier`
- `Fast`：传 `service_tier="fast"`
- `Flex`：传 `service_tier="flex"`
...
| `/modify [--provider claude\|openai] [--mode container\|host] [--workspace <绝对路径>] [--codex-service-tier <standard\|fast\|flex>] [--env KEY=VALUE] [--unset-env KEY]` | - | 修改当前绑定工作区的运行配置与环境变量；`--workspace` 仅在 `host` 模式下生效 |
| `/fromapp` | - | 查看并导入本地 Codex App 最近活跃线程，创建中转工作区 |
| `/toapp` | - | 将当前工作区结果回写到来源 Codex App 线程，并清理中转工作区 |
...
│   ├── sessions/{folder}/        #   会话持久化目录
│   │   ├── .claude/             #     Claude 会话持久化
│   │   └── codex-home/          #     Codex rollout / auth / state 持久化
...
| `DEFAULT_CODEX_MODEL` | 空 | 系统级 Codex 默认模型（新工作区 / 轻量查询链路可继承） |
| `DEFAULT_CODEX_THINKING_EFFORT` | 空 | 系统级 Codex 默认推理强度 |
| `DEFAULT_CODEX_SERVICE_TIER` | 空 | 系统级 Codex 默认服务档位；留空即 Standard |
