...
// IPC_DELIVERY_TIMEOUT_MS, log a warning so swallowed IPC messages are
// diagnosable from host logs.
  ...
  return (chatJid, chatName) => {
    ...
    if (existing) {
      ...
      if (!existing.created_by) {
        ...
        existing.target_main_jid = existing.target_main_jid || `web:${homeFolder}`;
